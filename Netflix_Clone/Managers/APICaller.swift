...
class APICaller {
    ...
    func getPopular(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.baseURL)/3/movie/popular?api_key=\(K.API_KEY)&language=en-US&page=1") else {return}
            ...
            let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { (data, _, error) in
                ...
                do {
                    let results = try JSONDecoder().decode(TrendingTitleResponse.self, from: data)
                    completion(.success(results.results)) //look if is just print(results)
                }
                catch {
                    completion(.failure(APIError.failedToGetData))
                }
    ...
    func getMovie(with query: String, completion: @escaping (Result<VideoElement, Error>) -> Void) {
        ...
        guard let url = URL(string: "\(Constants.YoutubeBaseURL)q=\(query)&key=\(K.YoutubeAPI_KEY)") else {return}
